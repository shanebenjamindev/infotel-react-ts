...
const ReservationForecast = () => {
  ...
  const filterData = (period: any) => {
    ...
    switch (period) {
      case "this-month":
        filteredData = reservationForecastMonth.filter((entry: any) => {
      ...
      case "3-months":
        ...
        break;
      case "6-months":
        const sixMonthsLater = new Date(currentDate.getMonth() + 6);
        filteredData = reservationForecastMonth.filter(
          (entry: any) => new Date(entry.from_date) >= sixMonthsLater
        );
        break;
      default:
